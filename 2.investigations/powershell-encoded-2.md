...
```yaml
Process Create:
... [Insert your Sysmon log block here] ...
```
> Figure 1: Description of what the screenshot proves.

## 4. Analysis & Triage Steps
* **Command Line Inspection:** Reviewed the process creation telemetry (Sysmon Event ID 1) to identify the presence of the `-EncodedCommand` argument.
* **Payload Decoding:** Decoded the observed Base64 string (`VwByAGkAdABl...`) to reveal the underlying plaintext execution payload:
  * *Decoded content:* `Write-Host 'SOC Analyst Portfolio Test - T1059.001'`
* **Parent-Child Process Correlation:** Traced the process lineage, confirming an interactive parent instance spawning an encoded child process.
