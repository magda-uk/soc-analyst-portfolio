# 🛡️ Evidence: Sysmon Event ID 8 (CreateRemoteThread)
### Windows GUI Subsystem Baseline — DWM to CSRSS Thread Creation
...
## 🖼️ Telemetry Artifacts

![Sysmon Event ID 8 - Event Viewer Details](screenshots/event-8-dwm-csrss-details.png)
*Figure 1: Windows Event Viewer capture displaying Sysmon Event ID 8 (`CreateRemoteThread`) between `dwm.exe` and `csrss.exe`.*

![Sysmon Event ID 8 - Raw XML Telemetry](screenshots/event-8-dwm-csrss-xml.png)
*Figure 2: Raw XML view verifying user contexts (`Window Manager\DWM-6` to `NT AUTHORITY\SYSTEM`) and start memory address.*
...
### 2. Authentication & Privilege Verification
...
To prevent alert fatigue and reduce false positives in SIEM/EDR platforms, high-volume legitimate OS subsystem interactions should be baseline-filtered at the Sysmon XML or SIEM query level.

### Sysmon XML Exclusion Filter Rule
```XML
<RuleGroup groupRelation="or" name="Benign System Thread Creation">
  <CreateRemoteThread onmatch="exclude">
    <!-- Exclude legitimate Desktop Window Manager to CSRSS thread activity -->
    <Rule groupRelation="and">
      ...
      <SourceUser condition="begin with">Window Manager\DWM-</SourceUser>
    </Rule>
  </CreateRemoteThread>
</RuleGroup>
```
